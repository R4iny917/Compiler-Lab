...
Operand translate_VarDec(struct Node* root){
    ...
    if(root->child_num == 1){
        ...
        if(type->kind == ARRAY){
            ...
            add_interCode(ic);
            add_interCode(new_InterCode(IC_ASSIGN,arr_op->u.op,new_addr(arr_op),NULL,NULL));
            return arr_op;
        }
    }
    if(root->child_num == 4){
    ...
    if(root->child_num == 4){// 处理有参函数
        ...
        while (params!=NULL)
        {
         // 构造函数参数列表判断参数类型（选做内容中需要支持结构体和数组作为参数）
            ...
            if(params->type->kind == ARRAY){ // 处理数组或者结构体
                ...
                add_interCode(new_InterCode(IC_PARAM,operand->u.op,NULL,NULL,NULL));
            }
            params = params->tail;
        }
        ...
        while(params != NULL){
            if(params->type->kind == ARRAY){
                ...
                add_interCode(new_InterCode(IC_ASSIGN,op,new_point(op->u.op),NULL,NULL));
            }
            params = params->tail;
        }
    }
...
void translate_Exp(struct Node* root, Operand place){ // place表示值
    ...
    if(strcmp(root->children[0]->name,"Exp") == 0){
        ...
        if(strcmp(root->children[1]->name,"LB") == 0){ //| Exp LB Exp RB
            ...
            add_interCode(new_InterCode(IC_PLUS,tmp3,tmp1->u.op,tmp3,NULL));
...
void translate_Args(struct Node* root, int write_func){
    ...
    if(write_func == true)//写函数
        ...
        if(t1->kind != OP_ARRAY)
            ...
            add_interCode(new_InterCode(IC_ARG,t1->u.op,NULL,NULL,NULL));
    }
}
